...
class MainActivity : AppCompatActivity(), ContactAdapter.OnContactActionListener {
    ...
    private val getEditImage = registerForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? ->
        uri?.let {
            editingImageUri = it
    ...
    private fun sortContacts() {
        if (contactList.isEmpty()) {
            Toast.makeText(this, "No contacts to sort", Toast.LENGTH_SHORT).show()
            return
        }
    ...
    private fun saveContact() {
        ...
        btnPickImage.setImageResource(android.R.drawable.ic_menu_camera)
        etName.requestFocus()
    }
    ...
    private fun showEditDialog(position: Int) {
        ...
        editingImageUri = contact.profileImageUri?.let { Uri.parse(it) }
        
        if (editingImageUri != null) {
            ivEditProfile.setImageURI(editingImageUri)
        } else {
            ivEditProfile.setImageResource(android.R.drawable.ic_menu_camera)
        }

        currentEditImageView = ivEditProfile
        ivEditProfile.setOnClickListener {
            getEditImage.launch("image/*")
        }
        ...
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
            ...
            if (validateInputs(updatedName, updatedPhone, etEditName, etEditPhone)) {
                ...
                contact.profileImageUri = editingImageUri?.toString()
                contactAdapter.updateList(contactList)
